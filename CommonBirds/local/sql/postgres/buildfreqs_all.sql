...
BEGIN
  -------------------- CREATE TABLES --------------------------------------
  -- Get rid of old type information
...
for this in select column_name, data_type, isnominal from public.icol_types where table_schema = schname and table_name = tabname
  loop
    ...
    if lower(this.column_name) not in ('ntag1', 'serno', 'sex', 'inf') then
       CASE this.data_type
         WHEN 'bigint', 'integer', 'smallint' THEN
	     nproc := nproc + 1;
             EXECUTE 'insert into public.frequencies (fieldname, fieldvalue, fieldcount) select * from dofreqs($1, $2, $3)'
             USING schname, tabname, this.column_name;
	 WHEN 'double precision' THEN
	     -- Multiply all the dp values by 100
	     nproc := nproc +1;
	     EXECUTE 'insert into public.frequencies (fieldname, fieldvalue, fieldcount) select * from dofreqsdp($1, $2, $3)'
             USING schname, tabname, this.column_name;
	 ELSE
	     CONTINUE;
       END CASE;
  ...
  -- Clean up likely missing values
  EXECUTE 'delete from public.frequencies where fieldvalue < 0 or fieldvalue > 7000';
