...
-- These are to be installed in the robin databasse on Kiwi
-- --------------------------------------------------------
DELIMITER ;;
...
CREATE DEFINER=`root`@`%`TRIGGER `catlab_ins_log` AFTER INSERT ON `categorylabels`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `catlab_upd_log` AFTER UPDATE ON `categorylabels`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `catlab_del_log` BEFORE DELETE ON `categorylabels`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `catmem_ins_log` AFTER INSERT ON `categorymembers`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `catmem_upd_log` AFTER UPDATE ON `categorymembers`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `catmem_del_log` BEFORE DELETE ON `categorymembers`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `vallabs_ins_log` AFTER INSERT ON `valuelabels`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `vallabs_upd_log` AFTER UPDATE ON `valuelabels`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `vallabs_del_log` AFTER DELETE ON `valuelabels`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `varlabs_ins_log` AFTER INSERT ON `variablelabels`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `varlabs_upd_log` AFTER UPDATE ON `variablelabels`
  FOR EACH ROW
    BEGIN
...
CREATE DEFINER=`root`@`%` TRIGGER `varlabs_del_log` AFTER DELETE ON `variablelabels`
  FOR EACH ROW
    BEGIN
    ...
    END ;;
DELIMITER ;
-- Dump completed on 2025-02-28 11:21:47
